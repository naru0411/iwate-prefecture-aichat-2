...
class RAGEngine:
    ...
    def _generate_answer(self, query, context, ref_urls):
        """厳密な生成とハルシネーション抑制ロジック (超厳格モード・改)"""
        
        system_prompt = """あなたは岩手県立大学の厳格な検証官です。以下の【禁止事項】を犯した場合、回答は失敗とみなされます。

【禁止事項】
1. **日付の自己矛盾**: 『10月下旬（1月26日）』のように、月が一致しない、または論理的に破綻している日付が含まれる場合、その情報は絶対に回答せず『NO_INFO』と出力せよ。
2. **表形式データの厳密読解**: 表データから情報を抽出する際、学部名（行）と日程（列）の対応が100%確実でない限り、回答を捏造してはならない。少しでも確信が持てなければ即座に『NO_INFO』を出力せよ。
3. **中国語の完全排除**: 『ソフトウェア』を『软件』と出力することは厳禁である。必ず正しい日本語を使用せよ。

【鉄則】
1. **情報の分離**: 資料内に『質問された学部以外の名前』が出てきた場合、その前後にある情報はすべて無視せよ。学部の境界をまたいで情報を統合してはならない。
2. **推論の禁止**: 資料に明記されていない組み合わせを作った瞬間、回答を中止し『NO_INFO』と出力せよ。
3. **自己検閲**: 回答文を作成した後、質問された学部の情報だけで構成されているか再確認せよ。"""
...
### 回答"""
        ...
        # ※異なる月が登場し、かつその間隔が3ヶ月以上離れている場合、情報の混同とみなす
        months = [int(m) for m in re.findall(r'(\d+)月', answer)]
        if len(set(months)) >= 2:
            sorted_months = sorted(list(set(months)))
            # 単純な最大差分チェック (年またぎは簡易的に無視するか、あるいは厳しく弾く)
            # 例: 9月と1月 -> 差は8 (絶対値)。しかし入試ではありうる。
            # ユーザーの「9月...1月」は明らかに別日程の混入を示唆している。
            # ここでは「3種類以上の月」または「特定の危険な組み合わせ」を弾くか？
            # ユーザー指示に従い、強制fallback
            # "矛盾している場合" -> 9月と1月が出たらアウトにする
    ...
    def _get_fallback_message(self):
        return "申し訳ありません。入試日程等の正確性が極めて重要な情報については、誤情報の提供を避けるため、現時点ではAIによる回答を制限しております。恐れ入りますが、以下の公式サイトより最新の募集要項を直接ご確認ください。"
